...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run an indicator-based BTCUSDT paper-trade loop using Binance candles."
    )
    ...
    parser.add_argument("--lookback", type=int, default=256, help="Number of candles to analyze.")
    ...
    parser.add_argument(
        "--log-file",
        default="logs/btcusdt_indicator_papertrade.jsonl",
...
def main() -> int:
    ...
    log_line(f"Starting indicator paper-trade loop for {args.symbol} @ {args.interval}")
    ...
    try:
        while True:
            ...
            log_line(
                f"Cycle {cycle_num}: {signal['signal']} "
                f"(score={signal['score']}, reasons={'; '.join(signal['reasons']) or 'none'})"
            )
